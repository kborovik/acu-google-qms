...
def emit_manifest_json(suite: InboundShipmentSuite, out_path: Path) -> Path:
    ...
    manifest_data = {
        ...
        "vendor_name": suite.vendor.legal_name,
        ...
        "shipping_documents": {
            "certificate_of_analysis": {
                "document_id": f"COA-{suite.manifest_id[4:]}",
                "testing_lab_id": suite.test_lab.lab_id,
                "testing_lab_name": suite.test_lab.lab_name,
                "document_standard": suite.test_lab.document_standard,
                "accreditation_number": suite.test_lab.accreditation_number,
                "verified_in_packet": True,
                "overall_evaluation": suite.overall_status,
            },
            "packing_slip": {
                "packing_slip_number": f"PACK-{suite.manifest_id[4:]}",
                "verified_in_packet": True,
                "destination_warehouse": suite.product.default_warehouse,
            },
            "bill_of_lading": {
                "bol_number": f"BOL-{suite.manifest_id[4:]}",
                "piece_count": suite.pallet_count,
        ...
        "line_items": [
            {
                ...
                "default_quarantine_location": (
                    suite.product.default_quarantine_location
                ),
            }
        ],
        "analytical_results": [
